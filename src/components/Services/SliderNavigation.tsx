  ...
  return (
    <ul className="flex flex-col gap-[16px] pt-[24px] md:pt-0 lg:gap-[24px]">
      {navigationList.map((el, index) => (
        <li key={el} className="flex items-center gap-[8px]">
          {activeIndex === index && (
            <div className="h-[6px] w-[6px] rotate-45 bg-white"></div>
          )}
          <button
            type="button"
          ...
          ${
            activeIndex === index
              ...
              : 'opacity-50 transition-all duration-300 hover:opacity-100 focus:font-medium focus:opacity-100 focus:outline-1 focus:outline-white'
          }`}
